/**
 * Methlang Syntax Extension - Advanced Linter
 *
...
function lintRegex(document) {
    ...
    for (const seg of segments) {
      ...
      const add = (re, msg) => {
        ...
        while ((m = r.exec(slice)) !== null) {
          ...
          diagnostics.push(mkDiag(lineIdx, col, m[0].length, msg, 'meth'));
        }
      };
  ...
  if (inString) {
    diagnostics.push(mkDiag(stringStartLine, stringStartCol, 1, 'Unterminated string literal. Add closing double quote.', 'meth'));
  }

  return diagnostics;
}

function mkDiag(line, col, len, msg, source = 'meth') {
...
function getConfig() {
  const cfg = vscode.workspace.getConfiguration('meth');
  return {
...
function findCompiler(workspaceRoot) {
  ...
  const candidates = [
    ...
    path.join(workspaceRoot, 'methlang.exe'),
    path.join(workspaceRoot, 'methlang'),
  ];
