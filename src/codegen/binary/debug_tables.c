...
int code_generator_binary_emit_crash_startup(CodeGenerator *generator) {
  ...
  size_t lea_functions_offset = 0;
  size_t lea_locations_offset = 0;
  DebugInfo *debug_info = NULL;
  ...
  debug_info = generator->debug_info;
  function_count = debug_info ? debug_info->runtime_function_count : 0;
  location_count = debug_info ? debug_info->runtime_location_count : 0;
  ...
  if (function_count > 0) {
    if (!binary_emit_lea_reg_rip_placeholder(&code, BINARY_GP_RCX,
                                             &lea_functions_offset) ||
        !binary_emit_mov_reg_imm64(&code, BINARY_GP_RDX,
                                   (unsigned long long)function_count) ||
        !binary_emit_lea_reg_rip_placeholder(&code, BINARY_GP_R8,
                                             &lea_locations_offset) ||
        !binary_emit_mov_reg_imm64(&code, BINARY_GP_R9,
                                   (unsigned long long)location_count) ||
        ...
        !binary_call_relocation_table_add(
            &call_relocations, "mettle_crash_register_image",
  ...
  if (function_count > 0) {
    if (!binary_emitter_add_relocation(
            emitter, text_section, function_offset + lea_functions_offset,
            BINARY_RELOCATION_REL32, "mettle_debug_functions", 0) ||
        !binary_emitter_add_relocation(
            emitter, text_section, function_offset + lea_locations_offset,
            BINARY_RELOCATION_REL32, "mettle_debug_locations", 0) ||
        !code_generator_binary_declare_external_symbol(
            generator, "mettle_crash_register_image")) {
      goto fail;
    }
  }
