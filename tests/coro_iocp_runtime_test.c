...
/*
 * Coroutine reactor + stackless task scheduler tests.
 *
 * The same logical scenarios run on both backends:
 *   - Windows: IOCP reactor (meth_coro_iocp_runtime_*).
 *   - POSIX:   poll(2) + self-pipe reactor (same public API).
 *
 * Only the test's own threading/sleep helpers differ per platform; every
 * TEST_ASSERT below exercises the platform-agnostic runtime contract.
 */
...
static void poll_thread_body(PollResult *poll) {
  if (!poll) {
    return;
  }
  poll->poll_rc =
      meth_coro_iocp_runtime_poll(1500, &poll->token, &poll->kind,
                                  &poll->result);
  poll->done = 1;
}
...
static DWORD WINAPI poll_thread_main(LPVOID parameter) {
  poll_thread_body((PollResult *)parameter);
  return 0u;
}

static int test_thread_start(TestThread *thread, PollResult *poll) {
  *thread = CreateThread(NULL, 0, poll_thread_main, poll, 0, NULL);
  return *thread != NULL;
}

static int test_thread_join(TestThread thread) {
  int ok = WaitForSingleObject(thread, 3000) == WAIT_OBJECT_0;
  CloseHandle(thread);
  return ok;
}
#else
static void *poll_thread_main(void *parameter) {
  poll_thread_body((PollResult *)parameter);
  return NULL;
}

static int test_thread_start(TestThread *thread, PollResult *poll) {
  return pthread_create(thread, NULL, poll_thread_main, poll) == 0;
}
...
static int test_iocp_init_shutdown_idempotent(void) {
  TEST_ASSERT(meth_coro_iocp_runtime_init() == 1, "reactor init failed");
  TEST_ASSERT(meth_coro_iocp_runtime_init() == 1,
              "reactor second init failed");
  TEST_ASSERT(meth_coro_iocp_runtime_shutdown() == 1,
              "reactor shutdown failed");
  TEST_ASSERT(meth_coro_iocp_runtime_shutdown() == 1,
              "reactor second shutdown failed");
  return 1;
}
...
static int test_iocp_poll_wake(void) {
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_init() == 1,
              "reactor init failed before poll wake test");

  TEST_ASSERT(test_thread_start(&thread, &poll), "failed to create poll thread");

  test_sleep_ms(20);
  TEST_ASSERT(meth_coro_iocp_runtime_post_wake((uintptr_t)0x1234u, 77) == 1,
              "post wake failed");

  TEST_ASSERT(test_thread_join(thread), "poll thread did not finish");
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_shutdown() == 1,
              "reactor shutdown failed after poll wake test");
...
static int32_t test_coro_step(void *state_ptr, uintptr_t wake_token,
  ...
  if (state->phase == 0) {
    state->phase = 1;
    if (meth_coro_iocp_runtime_post_wake((uintptr_t)state->task_handle, 55) !=
        1) {
      state->saw_error = 1;
      return 1;
    }
...
static int test_stackless_task_resume_via_iocp(void) {
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_init() == 1,
              "reactor init failed before stackless task test");
  ...
  TEST_ASSERT(state.saw_error == 0,
              "coroutine task saw wake metadata mismatch");
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_shutdown() == 1,
              "reactor shutdown failed after stackless task test");
  return 1;
}
...
static int test_stackless_task_bind_external_token(void) {
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_init() == 1,
              "reactor init failed before bind-token task test");
  ...
  TEST_ASSERT(meth_coro_task_bind_token(state.task_handle,
                                        (uintptr_t)0xBEEFu) == 1,
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_shutdown() == 1,
              "reactor shutdown failed after bind-token task test");
  return 1;
}
...
static int test_task_schedule_coalesces_latest_wake(void) {
  CoroState state = {0};
  TEST_ASSERT(meth_coro_iocp_runtime_init() == 1,
              "reactor init failed before coalesce test");
  ...
  TEST_ASSERT(state.saw_result == 99,
              "coalesce task did not see latest result");
  TEST_ASSERT(meth_coro_task_is_done(state.task_handle) == 1,
  ...
  TEST_ASSERT(meth_coro_iocp_runtime_shutdown() == 1,
              "reactor shutdown failed after coalesce test");
  return 1;
}
...
#ifdef _WIN32
  printf("Coroutine reactor runtime tests passed (IOCP backend).\n");
#else
  printf("Coroutine reactor runtime tests passed (POSIX poll backend).\n");
#endif
  return 0;
}
